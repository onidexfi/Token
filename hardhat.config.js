...
task('deploy', 'Deploy Paragen Token')
    .setAction(async () => {
        const [deployer] = await ethers.getSigners();

        console.log(`Deploying Paragen Token with the account: ${deployer.address}`);

        console.log(`Deployer balance: ${ethers.utils.formatEther(await deployer.getBalance())} BNB`);

        const Token = await ethers.getContractFactory('Paragen');
        ...
        console.log('Done!');
        console.log(`Deployer balance: ${ethers.utils.formatEther(await deployer.getBalance())} BNB`);
        console.log(`Deployer RGEN balance: ${ethers.utils.formatEther(await token.balanceOf(deployer.address))} RGEN`);
    });
...
task('revoke-blocked', 'Revoke tokens from blocked accounts')
    ...
    .setAction(async ({token: tokenAddress, json, to}) => {
        ...
        console.log(`Sender balance: ${ethers.utils.formatEther(await sender.getBalance())} BNB`);
