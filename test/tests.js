...
describe('Paragen (RGEN) Token With Protection', function() {
  ...
  const TOTAL_SUPPLY = 200000000n * EXT;
  const PROTECTION_FEE = 250000n * EXT;
  ...
  const ProtectionConfig = {
    ...
    LiquidityAmountTrap_amount: 37000n * EXT,
    ...
    TokensToPutIntoLiquidityPool: 1000000n * EXT,


    PredeployedLPS: '0x9786b0BeDC1fF41f4Dc1C471312ec1D179CBFa06',
    PredeployedLPSCore: '0xDE5f4Cd6AD5E1bF247418CaEC6BD19e4e6cF2c1d',
  ...
  before('snapshot', async function() {
    ...
    const ParagenToken = await ethers.getContractFactory('Paragen');
    ext = await ParagenToken.deploy();
    await ext.deployed();
  ...
  describe("Liquidity Protection Service", () => {
    ...
    it('Should trap all buyers in the first block', async function() {
      ...
      // Block user1.
      const tx1 = await addToBlock(() => ext.connect(lpSigner).transfer(user1, 3, GAS_LIMIT));
      // Should not be blocked yet.
      // Should infect the receiver.
      const infectTx = await addToBlock(() => ext.connect(user1Signer).transfer(user2, 2, GAS_LIMIT));
      ...
      await mine();
      await startMining();
      await expect(tx1).to.emit(lps, 'Blocked').withArgs(lp, user1, 'FirstBlockTrap');
      await expect(infectTx).to.emit(lps, 'Blocked').withArgs(lp, user2, 'Infected FirstBlockTrap');
      ...
      await expect(ext.connect(user2Signer).transfer(lp, 1))
          .to.be.revertedWith('FirstBlockTrap: blocked');
      await expect(ext.connect(user2Signer).transfer(user3, 1))
          .to.be.revertedWith('FirstBlockTrap: blocked');
      await ext.connect(lpSigner).transfer(user3, 1);
      await ext.connect(user3Signer).transfer(lp, 1);

      await ext.revokeBlocked([user1, user2, user3, lp], revoker);
      expect(await ext.balanceOf(revoker)).to.equal(6);
    ...
    it('Should trap all buyers who bought above amount limit', async function() {
      ...
      // Should infect the receiver.
      const infectTx = await addToBlock(() => ext.connect(user3Signer).transfer(user2, 1, GAS_LIMIT));
      await mine();
      // Should be blocked.
      ...
      // User2 blocked buys keep working.
      ...
      await expect(infectTx).to.emit(lps, 'Blocked').withArgs(lp, user2, 'Infected LiquidityAmountTrap');
      await expect(tx2.wait()).to.be.reverted;
      await expect(tx3).to.not.emit(lps, 'Blocked');
    ...
    it('Should trap all buyers who bought above percent limit', async function() {
      ...
      await expect(infectTx).to.emit(lps, 'Blocked').withArgs(lp, user2, 'Infected LiquidityPercentTrap');
      await expect(tx2.wait()).to.be.reverted;
      await expect(tx3).to.not.emit(lps, 'Blocked');
      await expect(ext.connect(user2Signer).transfer(user1, 1))
    ...
    it('Should trap all traders if more than 8 trades in the second block', async function() {
      ...
      await ext.revokeBlocked([user1, user2, user3, lp], revoker);
      expect(await ext.balanceOf(revoker)).to.equal(97 + (10*(ProtectionConfig.LiquidityActivityTrap_count - 2)) + 11);
      expect(await ext.balanceOf(user1)).to.equal(0);
      expect(await ext.balanceOf(user2)).to.equal(0);
      expect(await ext.balanceOf(user3)).to.equal(0);
    });
    ...
    it('Should disable protection', async function() {
      ...
      await expect(ext.connect(user1Signer).disableProtection())
          .to.be.reverted;
      await setTimeAndMine(1648425599)
